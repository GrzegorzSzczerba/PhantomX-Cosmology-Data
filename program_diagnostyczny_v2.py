...
def pobierz_dane():
    ...
    # URL do Twojego publicznego repozytorium (surowe pliki)
    ...
    for nazwa, url in pliki_do_pobrania.items():
        if not os.path.exists(nazwa):
            try:
                ...
                print(f" [!] Błąd pobierania {nazwa}. Upewnij się, że plik istnieje w repozytorium. Błąd: {e}")
        else:
            print(f" -> Plik {nazwa} już istnieje lokalnie.")

    # Generowanie referencyjnego pliku CMB dla Planck 2018 (Shift Parameters)
    # Ze względu na to, że to tylko 3 liczby, bezpieczniej wygenerować je w skrypcie 
    # jako "pobrany" wzorzec z bazy Planck Legacy Archive.
...
def get_chi2_cmb(Om_m0, z_star, R_obs, sigma_R):
    """
    Kluczowa funkcja: CMB Shift Parameter (R) likelihood.
    R = sqrt(Omega_m0) * int_0^{z_*} dz / E(z)
    """
    if Om_m0 <= 0 or Om_m0 >= 1:
        return np.inf
...
# WCZYTYWANIE DANYCH 
# =============================================================================
# (Pominięto rozbudowane parsowanie SNe/BAO dla czytelności z poprzedniego skryptu - działa analogicznie)
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    print("RAPORT DIAGNOSTYCZNY Z UWZGLĘDNIENIEM CMB")
    print("="*50)
    ...
    plt.title('Zgodność sond (BAO + SNe + CMB) dla modelu $\\rho_{DE} \\propto \dot{a}^{-2}$', fontsize=14, fontweight='bold')
    plt.grid(True, linestyle='--', alpha=0.5)
    plt.legend(fontsize=11, loc='upper left')
    plt.xlim(0.2, 0.4) 
    plt.tight_layout()
    plt.savefig('MCMC_Tension_Plot_with_CMB.png', dpi=300)
    print("-> WYKRES ZAPISANO JAKO: MCMC_Tension_Plot_with_CMB.png")